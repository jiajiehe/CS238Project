...
class Model(ModelDesc):
    def _get_inputs(self):
        ...
        return [InputDesc(tf.uint8, (None,) + IMAGE_SHAPE3, 'state'),
                ...
                InputDesc(tf.float32, (None,), 'action_prob1'),
                InputDesc(tf.float32, (None,), 'action_prob2')
                ]
    ...
    def _get_NN_prediction(self, image):
        ...
        h = PReLU('prelu', h)
        logits1 = FullyConnected('fc-pi_1', h, out_dim=NUM_ACTIONS, nl=tf.identity)    # unnormalized policy
        logits2 = FullyConnected('fc-pi_2', h, out_dim=NUM_ACTIONS, nl=tf.identity)    # unnormalized policy
        value1 = FullyConnected('fc-v_1', h, 1, nl=tf.identity) 
        value2 = FullyConnected('fc-v_2', h, 1, nl=tf.identity)
        return logits1, logits2, value1, value2

    def _build_graph(self, inputs):
        state, action, futurereward1, futurereward2, updateweight1, updateweight2, action_prob1, action_prob2 = inputs
        logits1, logits2, value1, value2 = self._get_NN_prediction(state)
        value1 = tf.squeeze(value1, [1], name='pred_value_1')  # (B,)
        value2 = tf.squeeze(value2, [1], name='pred_value_2')  # (B,)
        policy1 = tf.nn.softmax(logits1, name='policy_1')
        policy2 = tf.nn.softmax(logits2, name='policy_2')
        is_training = get_current_tower_context().is_training
        if not is_training:
            return
        log_probs1 = tf.log(policy1 + 1e-6)
        log_probs2 = tf.log(policy2 + 1e-6)

        log_pi_a_given_s1 = tf.reduce_sum(
            log_probs1 * tf.one_hot(action, NUM_ACTIONS), 1)
        log_pi_a_given_s2 = tf.reduce_sum(
            log_probs2 * tf.one_hot(action, NUM_ACTIONS), 1)
        ...
        pi_a_given_s1 = tf.reduce_sum(policy1 * tf.one_hot(action, NUM_ACTIONS), 1)  # (B,)
        pi_a_given_s2 = tf.reduce_sum(policy2 * tf.one_hot(action, NUM_ACTIONS), 1)  # (B,)
        
        importance1 = tf.stop_gradient(tf.clip_by_value(pi_a_given_s1 / (action_prob1 + 1e-8), 0, 10))
        importance2 = tf.stop_gradient(tf.clip_by_value(pi_a_given_s2 / (action_prob2 + 1e-8), 0, 10))
        importance_avg = tf.add(importance1 * 0.5, importance2 * 0.5)


        policy_loss1 = tf.reduce_sum(log_pi_a_given_s1 * advantage1 * importance1 * updateweight1, name='policy_loss_1')
        policy_loss2 = tf.reduce_sum(log_pi_a_given_s2 * advantage2 * importance2 * updateweight2, name='policy_loss_2')
        policy_loss = tf.add(policy_loss1, policy_loss2, name='policy_loss')
        
        xentropy_loss1 = tf.reduce_sum(policy1 * log_probs1 * updateweight1, name='xentropy_loss1')
        xentropy_loss2 = tf.reduce_sum(policy2 * log_probs2 * updateweight2, name='xentropy_loss2')
        xentropy_loss = tf.add(xentropy_loss1, xentropy_loss2, name='xentropy_loss')
        ...
        summary.add_moving_summary(policy_loss, xentropy_loss, value_loss,
                                   ...
                                   self.cost, tf.reduce_mean(importance_avg, name='importance'))
...
class MySimulatorMaster(SimulatorMaster, Callback):
    ...
    def _setup_graph(self):
        ...
        predictors = [self.trainer.get_predictor(
            ['state'], ['policy_1','policy_2','pred_value_1', 'pred_value_2'],
            self._gpus[k % nr_gpu])
    ...
    def _on_state(self, state, ident):
        def cb(outputs):
            ...
            client.memory.append(TransitionExperience(
                ...
                updateweight1=updateweight1, updateweight2=updateweight2, prob1=distrib1[action], prob2=distrib2[action]))
    ...
    def _on_datapoint(self, ident):
        ...
        if len(client.memory) == LOCAL_TIME_MAX + 1:
            R1 = client.memory[-1].value2 # add value from different branch of network
    ...
    def _parse_memory(self, init_r1, init_r2, ident, isOver):
        ...
        for idx, k in enumerate(mem):
            ...
            self.queue.put([k.state, k.action, R1, R2, k.updateweight1, k.updateweight2, k.prob1, k.prob2])
