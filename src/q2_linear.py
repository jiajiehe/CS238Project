...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        height, width, nchannels = state_shape
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        flatten_state = layers.flatten(state, scope=scope)
        out = layers.fully_connected(inputs=flatten_state, num_outputs=num_actions, 
            activation_fn=None, reuse=reuse, trainable=True, scope=scope)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        variables_q = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=q_scope)
        variables_target_q = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=target_q_scope)
        update_list = []
        for (v_target_q, v_q) in zip(variables_target_q, variables_q):
            update_list.append(tf.assign(v_target_q, v_q))
        self.update_target_op = tf.group(*update_list)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_loss_op(self, q, target_q):
        ...
        Q_samp = self.r + tf.cast(tf.logical_not(self.done_mask), tf.float32) * (self.config.gamma * target_q_max)
        indices = tf.one_hot(self.a, num_actions, 1.0, 0.0, axis=-1, dtype=tf.float32)
        q_a = tf.reduce_sum(q * indices, axis=1)
        self.loss = tf.reduce_mean((Q_samp - q_a) ** 2, axis=0)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_optimizer_op(self, scope):
        ...
        grad_v_list = optimizer.compute_gradients(self.loss, variables)
        if self.config.grad_clip:
            clipped_list = []
            for (grad, v) in grad_v_list:
                clipped_grad = tf.clip_by_norm(grad, self.config.clip_val)
                clipped_list.append((clipped_grad, v))
            grad_v_list = clipped_list
        grad_list = list(zip(*grad_v_list)[0])
        self.train_op = optimizer.apply_gradients(grad_v_list)
