...
from core.deep_q_learning_stoch import DQN
...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(dtype=tf.uint8, name='s', shape=(None, img_height, img_width, nchannels*self.config.state_history))
        self.a = tf.placeholder(dtype=tf.int32, name='a', shape=(None,))
        self.r = tf.placeholder(dtype=tf.float32, name='r', shape=(None,))
        self.sp = tf.placeholder(dtype=tf.uint8, name='sp', shape = (None, img_height, img_width, nchannels*self.config.state_history))
        self.done_mask = tf.placeholder(dtype=tf.bool, name='mask', shape=(None,))

        self.r_stoch = tf.placeholder(dtype=tf.float32, name='r_stoch', shape=(self.config.num_stoch_next_obs, None))
        self.sp_stoch = tf.placeholder(dtype=tf.uint8, name='sp_stoch', 
                                        shape = (self.config.num_stoch_next_obs, None, img_height, img_width, nchannels*self.config.state_history))
        self.done_mask_stoch = tf.placeholder(dtype=tf.bool, name='mask_stoch', shape=(self.config.num_stoch_next_obs, None))
        self.w_stoch = tf.placeholder(dtype=tf.float32, name='w_stoch', shape=(self.config.num_stoch_next_obs, None))
        self.w_e_stoch = tf.placeholder(dtype=tf.float32, name='w_e_stoch', shape=(self.config.num_stoch_next_obs, None))

        self.lr = tf.placeholder(dtype=tf.float32, name='lr', shape = ())
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        # (batch_size, img height, img width, nchannels) -> (batch_size, n)
        n = out.get_shape().as_list()[1]*out.get_shape().as_list()[2]*out.get_shape().as_list()[3] 
        out = tf.reshape(out, shape = [-1, n])

        with tf.variable_scope(scope, reuse=reuse):
            # initializer functions since they also default to what we want
            # activation_fn for fully_connected will default to ReLU, which is not always what we want.
            out = tf.contrib.layers.fully_connected(inputs = out, num_outputs = num_actions, activation_fn=None) # (batch_size, num_actions)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, q_scope)
        target_q = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, target_q_scope)
        assignment_list = [tf.assign(target_q[i], q[i]) for i in range(len(q))]
        self.update_target_op = tf.group(*assignment_list)
        ##############################################################
        ######################## END YOUR CODE #######################


    def add_loss_op(self, q, target_q, target_q_stoch):
        """
        ...
        samp_q = tf.add(self.r, tf.multiply(undone,tf.reduce_max(target_q, axis = -1))*self.config.gamma)
        '''
        undone_stoch = 1-tf.cast(self.done_mask_stoch, tf.float32)
        samp_q_stoch = tf.add(self.r_stoch, tf.multiply(undone_stoch,tf.reduce_max(target_q_stoch, axis = -1))*self.w_stoch) * self.w_e_stoch
        samp_q_stoch = tf.reduce_sum(samp_q_stoch, axis = 0)
        samp_q = (samp_q + samp_q_stoch) / (tf.reduce_sum(self.w_e_stoch, axis = 0) + 1.)
        '''
    ...
    def add_optimizer_op(self, scope):
        ...
        t_list = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope)
        grads_and_vars = optimizer.compute_gradients(self.loss, t_list)
        if self.config.grad_clip:
            grads_and_vars = [(tf.clip_by_norm(grad, self.config.clip_val), var) for grad, var in grads_and_vars]
        grad_list = list(zip(*grads_and_vars)[0])
