...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        rank_order = np.argsort(-self.p[:N])
        ...
        while True:
            ...
            for tmp_ind in tmp_idxes:
                if rank_order[tmp_ind] != (self.next_idx - 1) % self.size and tmp_ind not in idxes:
                    idxes[i] = tmp_ind
                    i += 1
                    if i >= batch_size:
                        break
    ...
    def store_frame(self, frame):
        ...
        if self.next_idx == 0:
            self.beta += (1.0 - self.config.beta) / (self.config.nsteps_train / self.size)
