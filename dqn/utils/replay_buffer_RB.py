...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        for ind in idxes:
            ...
            if self.sample_p[ind] > self.config.epsilon_p:
                propagate_p = self.sample_p[ind]
                self.sample_p[ind] = self.config.epsilon_p
                if ind != self.next_idx and (not self.done[prev_ind]) and (self.reward[prev_ind] == 0.):
                    self.sample_p[prev_ind] = propagate_p
                else:
                    next_ind = ind
                    end_ind = (self.next_idx - 1) % self.num_in_buffer
                    while next_ind != end_ind and (not self.done[next_ind]) and (self.reward[next_ind] == 0.):
                        next_ind = (next_ind + 1) % self.num_in_buffer
                    self.sample_p[next_ind] = max(propagate_p * self.config.lambda_p, self.config.epsilon_p)
        idxes = list(idxes)
